function [val,Rerror] = Systemerror(fieldx,fieldy,BS,R,group,f,c,N,lambda,erro,mode)
   ...
   %The receiver will wait a time interval equal to t for all signals
    ...
    for i = 1:size(group,1)
        for j = 1:size(group,2)
                %randn with mean = 0 e variance = erro
    ...
    Rerror = distance(grouperror,BSerror);
    ...
    if mode == 1
        ...
        plot3(BSerror(1),BSerror(2),BSerror(3),'s');
